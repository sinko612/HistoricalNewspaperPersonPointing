#!/usr/bin/env python3
"""
Evaluate the original Molmo model without a fine-tuned adapter on PeopleGator-style tasks.
The script generates answers for page images, parses localized outputs, and summarizes task-level metrics.
"""
...
PROMPT_ANY_PERSON_PHOTO = """
You are given a scanned historical newspaper page.
Point to the CENTER of one visible FACE that belongs to a person in a photograph.
Do not point to text, ornaments, drawings, or non-human figures.
Return EXACTLY one point in this format:
<point x=".." y=".."></point>
Coordinates must be 0-100 relative to the FULL page image.
...
def parse_args():
    ...
    ap = argparse.ArgumentParser(
        description="Evaluate pure/base Molmo on PeopleGator with aligned metrics."
    )
    ...
    ap.add_argument(
        "--detections-jsonl",
        default="",
        help="Raw PeopleGator detections JSONL with all face detections. Required for any_person_photo and point_all_person_photos; if omitted, a sibling people_gator__detections.jsonl is used when present.",
    )
    ap.add_argument(
    ...
    ap.add_argument(
        ...
        default="all",
        help="both = the two name-conditioned modes; all = any_person_photo plus both name-conditioned modes plus point_all_person_photos",
    )
    ...
    ap.add_argument(
        ...
        help="Use block for a manual whole-transformer-block map that is safer for Molmo on multiple GPUs.",
    )
...
def parse_max_memory(spec: str):
    """
    Parse a device-to-memory mapping used by Hugging Face model loading. The expected format is a comma-separated list such as cuda:0=20GiB,cpu=80GiB.
    """
    spec = (spec or "").strip()
    if not spec:
        return None
...
def build_molmo_block_device_map(num_gpus: int, num_blocks: int = 28) -> dict | None:
    """
    Build a layer-aware device map for Molmo transformer blocks.
    """
    if num_gpus < 1:
        return None
...
def merge_overlapping_detections(
    ...
    def union(a: int, b: int):
        """
        Merge two detection indices in the disjoint-set structure.
        """
        ra, rb = find(a), find(b)
        if ra != rb:
            parent[rb] = ra
...
def resolve_detections_path(args, ann_path: Path):
    ...
    if not det_path.exists():
        raise FileNotFoundError(
            "any_person_photo/point_all_person_photos need --detections-jsonl with all detections "
...
def sample_negative_names(
    ...
    """
    Sample names that are absent from the current page.
    """
...
def load_model_and_processor(args):
    ...
    """
    print("Loading processor from:", args.model_id)
    print("Evaluation variant: pure_base_molmo")
    processor = AutoProcessor.from_pretrained(
        args.model_id, trust_remote_code=True, torch_dtype="auto"
    )
    ...
    print("Loading pure/base model:", args.model_id)
    print(
        "device_map:",
        device_map,
        "max_memory:",
...
def save_failure_overlay(
    ...
    """
    Save a diagnostic overlay showing predictions, reference boxes, and the evaluated prompt.
    """
...
def eval_any_person_photo(
    ...
    """
    Evaluate whether the model can indicate that at least one person photograph is present.
    """
    prompt = PROMPT_ANY_PERSON_PHOTO
    raw, points, img_size = molmo_generate(model, processor, img_path, prompt, args)
    pred_none = int(is_none_response(raw))
    ...
    gt_anns = dedup_det_anns if dedup_det_anns else raw_det_anns
    p0 = pred_points_px[0] if pred_points_px else None

    row = {
        "eval_mode": "any_person_photo",
        "library": library,
        "document": document,
        "page_id": page_id,
        "image_path": str(img_path),
        "target_person_name": "__any_person_photo__",
        "expected_present": int(len(gt_anns) > 0),
        "predicted_none": pred_none,
        "n_faces_on_page": len(gt_anns),
        "n_gt_faces": len(gt_anns),
        "n_gt_faces_without_duplicates": len(dedup_det_anns),
        "n_gt_faces_strict_with_duplicates": len(raw_det_anns),
        "n_detections_merged_away": max(0, len(raw_det_anns) - len(dedup_det_anns)),
    ...
    if p0 is None or not gt_anns:
        return row, pred_points_px

    metrics = []
    for ann in gt_anns:
        d = dist_to_center(p0["x_px"], p0["y_px"], ann)
        hit = int(point_in_bbox(p0["x_px"], p0["y_px"], ann))
        metrics.append((ann, d, hit))
    ...
    row.update(
        {
            "success": int(any_hit),
            "hit_any_bbox": int(any_hit),
            "pred_x100": p0["x100"],
            "pred_y100": p0["y100"],
            "pred_x_px": p0["x_px"],
            "pred_y_px": p0["y_px"],
            "person_name_nearest": best_ann.get("person_name"),
            "nearest_gt_center_x": best_d["gt_center_x"],
            "nearest_gt_center_y": best_d["gt_center_y"],
            "nearest_gt_center_source": best_d["gt_center_source"],
            "dist_px_to_nearest": best_d["dist_px"],
            "dist_rel_nearest_bbox_diag": best_d["dist_rel_bbox"],
            "dist_px_to_nearest_target": best_d["dist_px"],
            "dist_rel_to_nearest_target_bbox": best_d["dist_rel_bbox"],
        }
    )
    ...
    for mode in sorted({r["eval_mode"] for r in rows}):
        ...
        if mode == "point_all_person_photos":
            ...
            def point_all_summary_for(prefix: str):
                """
                Build a metric dictionary for one subset of point_all_person_photos rows.
                """
        ...
        if mode == "any_person_photo":
            pos = [r for r in mode_rows if r.get("expected_present") == 1]
            dists = [
                float(r["dist_px_to_nearest"])
                for r in pos
                if r.get("dist_px_to_nearest") is not None
            ]
            summary["by_mode"][mode] = {
                "rows": len(mode_rows),
                "overall_success_percent": (
                ...
                "parse_fail_count": sum(r.get("parse_fail", 0) for r in mode_rows),
                "mean_dist_px_to_nearest_face": (
                    sum(dists) / len(dists) if dists else None
                ),
                "median_dist_px_to_nearest_face": (
                    statistics.median(dists) if dists else None
                ),
                "nearest_center_accuracy_px": center_accuracy_from_dist_rows(
                    pos, "dist_px_to_nearest", len(pos)
                ),
                "sum_gt_faces": sum(int(r.get("n_gt_faces", 0)) for r in mode_rows),
                "sum_pred_points": sum(
                    int(r.get("n_pred_points", 0)) for r in mode_rows
                ),
                "note": "Single-point any-person-photo prompt. GT comes from detections-jsonl and uses deduplicated detections when available.",
            }
            continue
...
def main():
    ...
    needs_any_person = "any_person_photo" in modes
    needs_point_all = "point_all_person_photos" in modes
    needs_detection_modes = needs_any_person or needs_point_all
    detection_mode_page_keys = set()
    detections_by_key = {}
    det_path = None
    if needs_detection_modes:
        detection_mode_page_keys = load_unique_page_keys(ann_path)
        det_path = resolve_detections_path(args, ann_path)
        print("Loading all detections for detection-based modes:", det_path)
        detections_by_key = load_peoplegator_detections(det_path)
        print(
            "Detection-mode page set from annotations:", len(detection_mode_page_keys)
        )
    ...
    for img_path, library, document, page_id in iter_export_images(data_root):
        ...
        raw_det_anns = []
        dedup_det_anns = []
        include_any_person = False
        include_point_all = False
        if needs_detection_modes and key in detection_mode_page_keys:
            ...
            if not raw_det_anns:
                ...
                include_any_person = needs_any_person
                if needs_point_all:
                    if len(raw_det_anns) > args.max_point_all_detections:
                        skipped_too_many_detections += 1
                    else:
                        include_point_all = True

        if not include_name and not include_any_person and not include_point_all:
        ...
        for mode in modes:
            if mode == "any_person_photo":
                if not include_any_person:
                    continue
                row, pred_points_px = eval_any_person_photo(
                    model,
                    processor,
                    args,
                    img_path,
    ...
    summary.update(
        {
            ...
            "any_person_photo_gt": "Uses detections-jsonl and deduplicated detections when available.",
            "modes": modes,
        }
    )
